...
export default class GameMap {
	...
	constructor(map: any) {
		...
		// x, y transpose
		this.tiles = this.tiles[0].map((_: any, colIndex: number) =>
			this.tiles.map((row: MapObject[]) => row[colIndex]),
		)
		//
		...
		for (let i = 0; i < this.height; i++) {
			for (let j = 0; j < this.width; j++) {
				if (this.tiles[i][j]['texture'] === 'chest') {
					...
					this.tiles[i + 1][j]['texture'] === 'wall' &&
					this.tiles[i - 1][j]['texture'] === 'wall' &&
					this.tiles[i][j + 1]['texture'] === 'wall' &&
					this.tiles[i][j - 1]['texture'] === 'wall'
				) {
					// all wall
					...
					this.tiles[i][j + 1]['texture'] === 'wall' &&
					this.tiles[i][j - 1]['texture'] !== 'wall' &&
					this.tiles[i - 1][j + 1]['texture'] === 'wall'
				) {
					// right up rihgtup wall
				...
				// update topTilesData
				this.topTilesData.push(
					this.tiles[i][j]['texture'] === 'wall'
						? this.tilesData[i * this.height + j]
						: null,
				)
			}
		}
